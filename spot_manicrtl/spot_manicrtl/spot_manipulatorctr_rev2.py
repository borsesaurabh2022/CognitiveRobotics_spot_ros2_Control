...
from spot_interface.action import ManipulatorAct
...
class MoveGroupActionClient(Node):
    def __init__(self):
        ...
        self.mani_actionsrv = ActionServer(self, ManipulatorAct, '/manipulator_actsrv', self.ManipulatoreReq_cb)
            ...
            if motion_plan_request_action_ready:
                if get_joint_angles_request_ready:
                    if not tf_calculated:
                        if counter == 0:
                            first_action = True
                            self.obj_frame_id = self.frame_id+"_psudomrk_gripper"
                        if counter == 1:
                            second_action = True
                            self.obj_frame_id = self.frame_id
                        # if bin_identifier:
